...
`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","dateDimension","d3","magnitudeDimension","magnitudeGroup","depthDimension","depthGroup","dataset","dtgHourDimension","dtgHourGroup"], function(md,container,dc,dateDimension,d3,magnitudeDimension,magnitudeGroup,depthDimension,depthGroup,dataset,dtgHourDimension,dtgHourGroup)
{
  const view = md`${container()}`
  
  const dataTable = dc.dataTable(view.querySelector('#dc-table-graph'))
  const magnitude   = dc.barChart(view.querySelector('#magnitude-chart'))
  const depth = dc.barChart(view.querySelector('#depth-chart'))
  const earthshake  = dc.lineChart(view.querySelector('#time-chart'))
  ...
  main.variable(observer("circles")).define("circles", ["circlesLayer","dataset","L"], function(circlesLayer,dataset,L)
{
  ...
  dataset.forEach( function(d) {
    let circle = null;
    circle = L.circle([d.latitude, d.longitude], d.magnitude * 1000, {
    color: '#fd8d3c',
    weight: 2,
    fillColor: '#fecc5c',
    fillOpacity: 0.5
...
depthDimension.group()
  ...
  main.variable(observer("dtgHourDimension")).define("dtgHourDimension", ["facts","d3"], function(facts,d3){return(
facts.dimension(d => d3.timeHour(d.dtg))
)});
  main.variable(observer("dtgHourGroup")).define("dtgHourGroup", ["dtgHourDimension"], function(dtgHourDimension){return(
dtgHourDimension.group()
)});
  return main;
}
